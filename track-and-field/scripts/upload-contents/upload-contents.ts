...
export const SPREADSHEET_EVENTS_NAMES = [
  '100m',
  '200m',
  '400m',
  '110mH',
...
export class SpreadsheetUploader {
  ...
  async uploadSheetImages() {
    ...
    const targetRows = await this._getUploadTargetRows(
      SPREADSHEET_COLORS_SHEET_NAME
    );
  ...
  private async _getUploadTargetRows(sheetName: string) {
    ...
    return this._doc.sheetsByTitle[sheetName]?.getRows().then((rows) => {
      return rows?.filter(
        (row) =>
          row[SPREADSHEET_UPLOAD_FLAG_ROW_HEADER] === 'TRUE' &&
          !!row[SPREADSHEET_ID_HEADER]
      );
    });
  }
  ...
  private async _generateSysIdMap() {
    ...
    await this._doc.loadInfo();
    const rows = await this._doc.sheetsByTitle[
      SPREADSHEET_MASTER_SHEET_NAME
    ]?.getRows();
    ...
    for (const [index, row] of targetRow.entries()) {
      ...
      this.sysIdMap.set(
        row[SPREADSHEET_ID_HEADER],
        row[SPREADSHEET_CONTENTFUL_SYS_ID_HEADER]
      );
  ...
  private async _uploadRowImages(row: GoogleSpreadsheetRow) {
    ...
    return await Promise.all(
      [...Array(SPREADSHEET_COLOR_IMAGE_COUNT)].map(async (_, i) => {
        row[`${SPREADSHEET_UPLOADED_IMG_HEADER_PREFIX}${i + 1}`] =
          await this._generateUrlAndUploadImage(
            row[`img${i + 1}`],
            `${SPREADSHEET_COLORS_SHEET_NAME}/${row[SPREADSHEET_ID_HEADER]}/${
              row[SPREADSHEET_COLOR_HEADER]
            }/img${i + 1}`
          );
      })
    ).then(async () => {
   ...
   */
  private async _generateUrlAndUploadImage(
    rawUrl: string,
    uploadFilePathWithoutExt: string
  ) {
    if (!rawUrl) {
      return undefined;
    }
  ...
  private async _uploadDriveToS3(
    driveFileId: string,
  ...
  private async _getDriveFileBuffer(
    driveFileId: string
  ...
  private async _uploadToS3(driveFileBuffer: Buffer | undefined, key: string) {
    ...
    // アップロード前に圧縮
    const reducedBuffer = await SpreadsheetUploader._reduceImageSize(
      driveFileBuffer
    );
  ...
  private async _updateTargetEntries(targetRows: GoogleSpreadsheetRow[]) {
    ...
    const articleRows = await this._doc.sheetsByTitle[
      SPREADSHEET_ARTICLE_SHEET_NAME
    ]?.getRows();
    ...
    for (const [index, targetRow] of targetRows.entries()) {
      ...
      const targetArticleRow = articleRows?.find(
        (articleRow) =>
          articleRow[SPREADSHEET_ID_HEADER] === targetRow[SPREADSHEET_ID_HEADER]
      );
  ...
  private async _updateEntry(
    ...
    targetArticleRow?: GoogleSpreadsheetRow
  ) {
    ...
    articleEntry.fields = await this._genarateArticleEntryFields(
      targetRow,
      false,
      targetArticleRow
    );

    await Promise.all([entry.update(), articleEntry.update()]).then(
      async ([updatedEntry, updatedArticleEntry]) => {
        if (
  ...
  private async _createRowArticleEntry(row: GoogleSpreadsheetRow) {
    ...
    return await this._contentfulEnvironment.createEntry(
      CONTENTFUL_ARTICLE_CONTENT_MODEL_ID,
      {
        fields
      }
    );
  ...
  private async _genarateEntryFields(
    ...
    const colorsFields = await this._generateColorsFields(
      row[SPREADSHEET_ID_HEADER]
    );
    ...
    return {
      ...
      alias: {
        'ja-JP': row?.alias
          ?.split(',')
          ?.flatMap((text: string) => (text ? text?.trim() : []))
      },
      spikeArticle: {
        'ja-JP': {
          sys: {
  ...
  private async _genarateArticleEntryFields(
    ...
    const keyFeaturesImageFields =
      await this.uploadAndGenerateKeyfeaturesImagesFields(articleRow);
  ...
  private async uploadAndGenerateKeyfeaturesImagesFields(
    articleRow: GoogleSpreadsheetRow
  ): Promise<{
    [key in keyof Partial<ISpikeArticlesFields>]: { 'ja-JP': any };
  }> {
    ...
    for (let count = 1; count < SPREADSHEET_KEY_FEATURES_COUNT + 1; count++) {
      keyfeaturesImageFields[
        `keyFeatureImageUrls${count}` as keyof ISpikeArticlesFields
      ] = {
        'ja-JP': await this._uploadAndGetKeyFeatureImageUrl(
          articleRow[SPREADSHEET_ID_HEADER],
          [
            articleRow[
              `${SPREADSHEET_ARTICLE_IMG_URL_HEADER_PREFIX}${count}_1`
            ],
  ...
  private _generateStrengthFieldValue(
    articleRow: GoogleSpreadsheetRow
  ): IShoeStrength[] {
    return [...Array(SPREADSHEET_STRENGTH_COUNT)].flatMap((_, i) =>
      articleRow[`strengthTitle${i + 1}`]
        ? {
            icon: SpreadsheetUploader._getIconClass(
              articleRow[`strengthIcon${i + 1}`]
            ),
  ...
  static getlevel(levelLabel: string): AthleteLevelCode | undefined {
    if (!isString(levelLabel)) {
      return undefined;
    }

    return (
      Object.values(athleteLevels).find((level) => level.label === levelLabel)
        ?.id || undefined
    );
  }
  ...
  static getPinType(shoeLaceTypeText: string): string[] | undefined {
    switch (shoeLaceTypeText) {
      case 'fixed':
        return [shoePinType.fixed.id];
      case 'removable':
        return [shoePinType.removable.id];
      case 'both':
        return [shoePinType.fixed.id, shoePinType.removable.id];
      default:
        return undefined;
    }
  }
